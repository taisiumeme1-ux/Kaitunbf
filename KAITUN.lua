...
_G.KaitunUI = API
-- =========================================================
-- [PLUGIN] TÍCH HỢP AUTO GODHUMAN & RAID (DÁN VÀO ĐÂY)
-- =========================================================

local MeleeShop = {
    ["Superhuman"] = {Pos = Vector3.new(-1106, 331, -4432), Remote = "BuySuperhuman"},
    ["Death Step"] = {Pos = Vector3.new(-5414, 15, -5303), Remote = "BuyDeathStep"},
    ["Sharkman Karate"] = {Pos = Vector3.new(-1101, 15, -10503), Remote = "BuySharkmanKarate"},
    ["Electric Claw"] = {Pos = Vector3.new(-5274, 15, -1286), Remote = "BuyElectricClaw"},
    ["Dragon Talon"] = {Pos = Vector3.new(-3255, 300, -9725), Remote = "BuyDragonTalon"},
    ["Godhuman"] = {Pos = Vector3.new(-2855, 239, -13346), Remote = "BuyGodhuman"}
}

function CheckOwned(name)
    local inv = replicated.Remotes.CommF_:InvokeServer("getInventory")
    for _, v in pairs(inv) do if v.Name == name then return true end end
    return false
end
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            ...
            -- 1. ƯU TIÊN ĐI RAID NẾU THIẾU FRAG (Sử dụng Fast Attack, No Skill)
            if myFrag < getgenv().Config.MinFragment and getgenv().Config.AutoRaid then
                _G.fykaitun = false -- Tắt farm level gốc của bạn
                if workspace:FindFirstChild("SeaBeast") or workspace.Map:FindFirstChild("Island1") then
                    -- Logic Fast Attack Raid
                    local target = nil
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                    ...
                    if target then
                        weaponSc("Melee") -- Gọi hàm cầm vũ khí của bạn
                        Root.CFrame = target.HumanoidRootPart.CFrame * CFrame.new(0, 20, 0)
                        -- Click cực nhanh (Fast Attack)
                        game:GetService("VirtualUser"):CaptureController()
                        game:GetService("VirtualUser"):Button1Down(Vector2.new(0,0), workspace.CurrentCamera.CFrame)
                        target.HumanoidRootPart.CFrame = Root.CFrame * CFrame.new(0,0,-5)
                    end
                else
                    -- Đi mua Chip & Vào Raid
                    local lab = (game.PlaceId == 7449423635) and Vector3.new(-5030, 315, -2820) or Vector3.new(-6411, 250, 4490)
                    if (Root.Position - lab).Magnitude > 50 then
                        _tp(CFrame.new(lab)) -- Dùng hàm teleport của bạn
                    else
            ...
            -- 2. KIỂM TRA MASTERY MELEE (Cày lên 400)
            ...
            if curMelee and curMelee.ToolTip == "Melee" and curMelee.Level.Value < getgenv().Config.MasteryLimit then
                _G.fykaitun = true -- Bật lại farm level để script gốc của bạn đi đánh quái
                _G.SelectWeapon = curMelee.Name
                return
            end

            -- 3. ĐI MUA VÕ MỚI
            for name, info in pairs(MeleeShop) do
                if not CheckOwned(name) and myBeli >= 3000000 then
                    _G.fykaitun = false
                    _tp(CFrame.new(info.Pos))
                    if (Root.Position - info.Pos).Magnitude < 20 then
                        replicated.Remotes.CommF_:InvokeServer(info.Remote)
                    end
                    break
                end
            end
